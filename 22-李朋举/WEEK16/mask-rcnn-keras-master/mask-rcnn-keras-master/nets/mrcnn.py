...
def get_predict_model(config):
    ...
    # ------------------------------------#
    #   【1.卷积部分】 Resent101 + FPN
    # ------------------------------------#
    # 获得Resnet里的压缩程度不同的一些层 Resent101部分
    _, C2, C3, C4, C5 = get_resnet(input_image, stage5=True, train_bn=config.TRAIN_BN)

    # FPN部分
    # 组合成特征金字塔的结构
    # P5长宽共压缩了5次
    # Height/32,Width/32,256   C5 -> P5 (参考PPT图)
    ...
    # P2, P3, P4, P5, P6可以用于获取建议框(P6只在训练当中使用) -> rpn_feature_maps训练当中使用
    rpn_feature_maps = [P2, P3, P4, P5, P6]
    # P2, P3, P4, P5用于获取mask信息(推理当中使用) -> mrcnn_feature_maps推理当中使用
    ...
    for p in rpn_feature_maps:
        logits, classes, bbox = rpn([p])  # 分别对5个特征层做rpn拿到各自的logits, classes, bbox
        rpn_class_logits.append(logits)   # 加到一起
    ...
    # ----------------------------------------------------------------------------#
    #   【ROIAlign部分(最终的预测框) + FC部分(分类+回归) 获得classifier的结果】
    #    结果1、2 分类+回归
    # ----------------------------------------------------------------------------#
