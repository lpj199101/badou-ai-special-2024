...
定义了一个名为 `generate_pyramid_anchors` 的函数，用于生成不同特征层的锚框（anchors），并将它们堆叠在一起:
    1. `scales`：这是一个列表，包含了不同特征层的尺度（scale）值。
    2. `ratios`：这是一个列表，包含了锚框的宽高比（aspect ratio）。
    3. `feature_shapes`：这是一个列表，包含了不同特征层的特征形状。
    4. `feature_strides`：这是一个列表，包含了不同特征层的特征步长。
    5. `anchor_stride`：这是锚框的步长。
    
    6. 函数使用一个循环遍历不同的特征层。
    7. 在每次循环中，通过调用 `generate_anchors` 函数生成该特征层的锚框。
    8. `generate_anchors` 函数根据给定的尺度、宽高比、特征形状和特征步长来生成锚框。
    9. 生成的锚框被添加到 `anchors` 列表中。
    10. 最后，使用 `np.concatenate` 函数将所有特征层的锚框堆叠在一起，形成一个大的锚框数组。
总的来说，这个函数的目的是根据不同的特征层参数生成锚框，并将它们组合成一个统一的锚框数组，以便在目标检测等任务中使用。这些锚框可以用于在不同的特征层上检测目标的位置和大小。
'''
...
定义了一个名为 `compute_backbone_shapes` 的函数，用于计算主干特征提取网络的形状。
    1. `config`：这是一个配置对象，可能包含了关于主干网络的各种设置，例如网络类型、步长等。 
    2. `image_shape`：这是输入图像的形状，通常是一个元组，表示图像的高度、宽度和通道数。
...
def get_anchors(config, image_shape):
    # 计算 backbone 网络的输出形状  [[256 256], [128 128], [ 64  64], [ 32  32], [ 16  16]]
